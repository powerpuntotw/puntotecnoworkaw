...
export const AdminReports = () => {
    ...
    const fetchAnalytics = async () => {
        try {
            setLoading(true);
            const ordersRes = await databases.listDocuments(import.meta.env.VITE_APPWRITE_DATABASE_ID, 'orders', [Query.limit(1000)]);
            const orders = ordersRes.documents;
            const revenue = orders.reduce((sum, o) => sum + (o.total_amount || 0), 0);
            setStats({ totalOrders: orders.length, totalRevenue: revenue, avgTicket: orders.length > 0 ? revenue / orders.length : 0, activeLocals: new Set(orders.map(o => o.location_id)).size });
            const daily = {};
            orders.forEach(o => { const d = new Date(o.$createdAt).toLocaleDateString(); if (!daily[d]) daily[d] = { date: d, orders: 0, revenue: 0 }; daily[d].orders++; daily[d].revenue += o.total_amount || 0; });
            setDailyData(Object.values(daily).slice(-7));
            const locals = {};
            orders.forEach(o => { const n = o.location_name || 'Desconocido'; if (!locals[n]) locals[n] = { name: n, revenue: 0 }; locals[n].revenue += o.total_amount || 0; });
            setLocalData(Object.values(locals));
            const sm = {};
            orders.forEach(o => { const s = o.status || 'pendiente'; if (!sm[s]) sm[s] = { name: s, value: 0 }; sm[s].value++; });
        ...
        <div className="space-y-6 pb-10">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
                {[
                    ...
                    { label: 'Ticket Prom.', value: `$${stats.avgTicket.toFixed(0)}`, icon: TrendingUp, color: 'text-warning' },
                    { label: 'Locales', value: stats.activeLocals, icon: MapPin, color: 'text-secondary' },
                ].map((kpi, idx) => (
                    <div key={idx} className="bg-card/50 backdrop-blur-xl border border-white/10 rounded-2xl p-4 sm:p-6 shadow-glow">
            ...
            {/* Gráficos: 1 col mobile, 2 col lg */}
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div className="bg-card/50 backdrop-blur-xl border border-white/10 rounded-2xl p-4 sm:p-6 shadow-glow">
                    <h3 className="text-base font-bold text-white mb-4">Tendencia de Ingresos</h3>
                    <div className="h-56 sm:h-72">
                        <ResponsiveContainer width="100%" height="100%">
                            <LineChart data={dailyData}>
                                ...
                                <Line type="monotone" dataKey="revenue" stroke="#EB1C24" strokeWidth={2} dot={{ r: 3, fill: '#EB1C24' }} />
                            </LineChart>
                        </ResponsiveContainer>
                    </div>
                </div>
                        ...
                        <div className="space-y-2 w-full sm:w-1/2">
                            {statusData.map((s, idx) => (
                                <div key={idx} className="flex items-center gap-2 text-xs">
                                    ...
                                    <span className="text-gray-400 capitalize">{s.name}: <strong className="text-white">{s.value}</strong></span>
                                </div>
                            ))}
                        </div>
                    </div>
                </div>
                <div className="bg-card/50 backdrop-blur-xl border border-white/10 rounded-2xl p-4 sm:p-6 shadow-glow lg:col-span-2">
                    ...
                    <div className="h-56 sm:h-72">
                        <ResponsiveContainer width="100%" height="100%">
                            <BarChart data={localData}>
                                ...
                                <YAxis stroke="#666" fontSize={9} tickLine={false} axisLine={false} />
                                <Tooltip contentStyle={tt} />
                                <Bar dataKey="revenue" fill="#0093D8" radius={[6, 6, 0, 0]} />
                            </BarChart>
                        </ResponsiveContainer>
                    </div>
                </div>
