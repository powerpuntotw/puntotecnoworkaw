...
export const Landing = () => {
    ...
    const { platformName, logoMain, logoLight, getLogoUrl } = useBranding();
    ...
    // Usa logoLight para el nav (fondo oscuro), o logoMain como fallback
    const navLogoId = logoLight || logoMain;
    const navLogoUrl = navLogoId ? getLogoUrl(navLogoId) : null;
        ...
        <div className="min-h-screen bg-background text-foreground overflow-hidden font-['Space_Grotesk',sans-serif]">
            ...
            <footer className="border-t border-white/5 py-8 px-6">
                <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4">
                    <div className="flex items-center gap-3">
                        {navLogoUrl ? (
                            <img src={navLogoUrl} alt={displayName} className="w-8 h-8 object-contain rounded-lg" />
                        ) : (
                            <div className="w-8 h-8 rounded-lg bg-gradient-hero flex items-center justify-center font-bold text-sm text-white">
                                {displayName.charAt(0).toUpperCase()}
                            </div>
                        )}
                        <span className="text-sm font-semibold text-gray-400">{displayName}</span>
                    </div>
                    <p className="text-xs text-gray-600">
                        © {new Date().getFullYear()} {displayName} · Plataforma de impresión inteligente
                    </p>
                    <p className="text-xs text-gray-700">Powered by Punto Tecnowork v3</p>
                </div>
            </footer>
        </div>
    );
